...
import { ArrowDownRight, CheckCircle2, XCircle } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Review user add-funds requests, verify payment references, & approve wallet credits.</p>
        </div>
      ...
      {loading ? (
        ...
        <Table headers={['Request ID', 'User', 'Amount', 'Method', 'Reference No', 'Status', 'Actions']}>
          {recharges.map((r) => (
            <tr key={r._id} className="hover:bg-brand-card/50 transition-colors">
              ...
              <td className="px-5 py-3 text-slate-300">{r.paymentMethod}</td>
              <td className="px-5 py-3 font-mono text-slate-400">{r.referenceNumber}</td>
